...
def validate_booking_data(booking_data: PaymentIntentRequest, db: Session):
    """Shared validation — returns (cabin, nights, total)."""
...
def create_payment_intent(req: PaymentIntentRequest, db: Session = Depends(get_db)):
    """Create a Stripe PaymentIntent for a 50% deposit."""
...
async def stripe_webhook(request: Request, db: Session = Depends(get_db)):
    """Handle Stripe webhook — creates booking after successful payment."""
    ...
    if event["type"] == "payment_intent.succeeded":
        ...
        # Check no duplicate was already created
